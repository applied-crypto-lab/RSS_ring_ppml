...
void minionn_mp(NodeNetwork *nodeNet, NodeConfiguration *nodeConfig, string model_path, uint size, uint batch_size) {
    ...
    Lint **W1 = new Lint *[numShares];
    Lint **W2 = new Lint *[numShares];
    Lint **W3 = new Lint *[numShares];
    Lint **W4 = new Lint *[numShares];
    Lint **b1 = new Lint *[numShares];
    Lint **b2 = new Lint *[numShares];
    Lint **b3 = new Lint *[numShares];
    Lint **b4 = new Lint *[numShares];
    Lint **X = new Lint *[numShares];
    Lint **Y = new Lint *[numShares];

    // vars for intermediate calculations
    Lint **X0 = new Lint *[numShares];        // convolution
    Lint **X1 = new Lint *[numShares];        // matmult
    Lint **X1_prime = new Lint *[numShares];  // matmult
    Lint **X1_prime2 = new Lint *[numShares]; // matmult
    Lint **X2 = new Lint *[numShares];        // maxpool
    Lint **X3 = new Lint *[numShares];        // convolution
    Lint **X3p = new Lint *[numShares];       // convolution
    Lint **X4 = new Lint *[numShares];        // matmult
    Lint **X5 = new Lint *[numShares];        // maxpool
    Lint **X6 = new Lint *[numShares];        // matmult
    Lint **X7 = new Lint *[numShares];        // matmult
    ...
    // ring_size = 48;
    ring_size = 20;
    splitData_mp(W1, W1_in, 16, 25, ring_size, nodeNet);
    splitData_mp(b1, b1_in, 1, 16, ring_size, nodeNet);

    ring_size = 30;
    splitData_mp(W2, W2_in, 16, 400, ring_size, nodeNet);
    splitData_mp(b2, b2_in, 1, 16, ring_size, nodeNet);

    ring_size = 40;
    splitData_mp(W3, W3_in, 100, 256, ring_size, nodeNet);
    splitData_mp(b3, b3_in, 1, 100, ring_size, nodeNet);

    // ring_size = 49;
    splitData_mp(W4, W4_in, 10, 100, final_ring_size, nodeNet);
    splitData_mp(b4, b4_in, 1, 10, final_ring_size, nodeNet);
    splitData_mp(Y, Y_in, 1, 10000, final_ring_size, nodeNet);
    ...
    for (k = 0; k < (size / batch_size); k++) {
        ...
        // trivial operation
        splitData_X_mp(X, X_in, 784, batch_size, k, ring_size, nodeNet);
        // printf("splitX done\n");
        ...
        Rss_MatMultArray_batch_mp(X1, W1, X0, 16, 25, 576, ring_size, batch_size, 0, 1, nodeNet);
        add_biases(X1, X1, b1, 16, 576, batch_size, nodeNet);

        ReLU_mp(X1, X1, 16 * 576 * batch_size, ring_size, nodeNet, 1);
        MaxPool_mp(X2, X1, 16, 24, 24, batch_size, ring_size, nodeNet, 1);
        // Rss_Convert_mp(X2, X2, 16 * 144 * batch_size, ring_size, ring_size + 10, nodeNet);
        new_Rss_Convert_mp(X2, X2, 16 * 144 * batch_size, ring_size, ring_size + 10, nodeNet);
        ...
        // W3 --> 30, X3 --> 20
        //  W2 stays constant
        Rss_MatMultArray_batch_mp(X4, W2, X3, 16, 400, 64, ring_size, batch_size, 0, 1, nodeNet);
        add_biases(X4, X4, b2, 16, 64, batch_size, nodeNet);

        ReLU_mp(X4, X4, 16 * 64 * batch_size, ring_size, nodeNet, 1);
        MaxPool_mp(X5, X4, 16, 8, 8, batch_size, ring_size, nodeNet, 1);
        new_Rss_Convert_mp(X5, X5, 16 * 4 * 4 * batch_size, ring_size, ring_size + 10, nodeNet);

        ring_size = 40;
        Rss_MatMultArray_batch_mp(X6, W3, X5, 100, 256, 1, ring_size, batch_size, 0, 1, nodeNet);
        add_biases(X6, X6, b3, 100, 1, batch_size, nodeNet);

        ReLU_mp(X6, X6, 100 * 1 * batch_size, ring_size, nodeNet, 1);
        Rss_Convert_mp(X6, X6, 100 * 1 * batch_size, ring_size, final_ring_size, nodeNet);

        Rss_MatMultArray_batch_mp(X7, W4, X6, 10, 100, 1, final_ring_size, batch_size, 0, 1, nodeNet);
        add_biases(X7, X7, b4, 10, 1, batch_size, nodeNet);

        gettimeofday(&end, NULL); // stop timer here
        timer += 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;

        arg_max_mp(X1, X0, X7, final_ring_size, 10, batch_size, nodeNet);
        Rss_Open_mp(res, X0, batch_size, final_ring_size, nodeNet);
