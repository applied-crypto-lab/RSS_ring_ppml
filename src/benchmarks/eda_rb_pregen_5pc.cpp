    ...
    for (size_t j = 0; j < num_iterations; j++) {
        ...
        while (true) {
            if (temp_rb_size - opt_rb_batch > 0) {
                ...
                Rss_RandBit_mp(out_rb, opt_rb_batch, ring_size, nodeNet);
                temp_rb_size -= opt_rb_batch;
            } else {
                // printf("RB : remainder = %i\n", temp_rb_size);
                Rss_RandBit_mp(out_rb, temp_rb_size, ring_size, nodeNet);
                break;
            }
        }
        while (true) {
            if (temp_eda_size - opt_eda_batch > 0) {
                ...
                Rss_edaBit_mp(out_eda, out_eda_bw, opt_eda_batch, ring_size, nodeNet);
                temp_eda_size -= opt_eda_batch;
            } else {
                // printf("EDA : remainder = %i\n", temp_eda_size);
                Rss_edaBit_mp(out_eda, out_eda_bw, temp_eda_size, ring_size, nodeNet);
                break;
            }
        }
