#include "../include/q_mobilenet_5pc.h"

void q_mobilenet_mp(NodeNetwork *nodeNet, NodeConfiguration *nodeConfig, string in_model_path, uint num_iterations, uint batch_size, uint alpha_index, uint input_dim, uint num_discarded) {
    // batch_size = num_iterations;
    // string mobile_path = "/projects/rss_nn/models/q_mobile/";
    ...
    // cout << "all_names.num_iterations() : " << all_names.num_iterations() << endl;
    ...
    for (size_t i = 0; i < num_layers; i++) {
        weights[i] = new Lint *[numShares];
        biases[i] = new Lint *[numShares];
    ...
    for (size_t i = 0; i < num_layers + 1; i++) {
        intermediate_x[i] = new Lint *[numShares];
    ...
    Lint **relu_bounds = new Lint *[numShares];
    Lint **trunc_vals = new Lint *[numShares];
    Lint **input_zp = new Lint *[numShares];
    Lint **weights_zp = new Lint *[numShares];
    Lint **output_zp = new Lint *[numShares];

    Lint **input_x = new Lint *[numShares];
    Lint **output_x = new Lint *[numShares];

    Lint **final_x = new Lint *[numShares];     // used for computing argmax
    Lint **final_x_ind = new Lint *[numShares]; // used for computing argmax
    ...
    for (size_t k = 0; k < num_iterations; k++) {
        ...
        Rss_conv2d_first_mp(intermediate_x, input_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, 3, &in_channels, alpha_param * 32, 2, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        ...
        // cout<<"layer_ctr : "<<layer_ctr<<endl;
        // gettimeofday(&start_layer, NULL); //start timer here
        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 64, 1, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        gettimeofday(&end_layer, NULL); // stop timer here
        // timer_layer = 1e6 * (end_layer.tv_sec - start_layer.tv_sec) + end_layer.tv_usec - start_layer.tv_usec;
        // printf("--- layer_%u time = %.6lf\n", layer_ctr, (double)(timer_layer * 1e-6));
        ...
        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 128, 2, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 128, 1, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 256, 2, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 256, 1, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 512, 2, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 512, 1, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;
        ...
        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 1024, 2, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_dw_pw_conv2d_mp(intermediate_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, &in_channels, alpha_param * 1024, 1, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_avg_pool_trunc_mp(intermediate_x[layer_ctr], intermediate_x[layer_ctr - 1], in_channels, &n, 0, 0, batch_size, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;

        Rss_conv2d_final_layer_mp(intermediate_x, output_x, weights, biases, relu_bounds, trunc_vals, input_zp, weights_zp, output_zp, &n, 1, &in_channels, 1001, 1, batch_size, &layer_ctr, public_M, ring_size, nodeNet, eda_timer);
        // cout<<"layer_ctr : "<<layer_ctr<<endl;
        ...
        eda_arg_max_time_mp(final_x, final_x_ind, intermediate_x[layer_ctr], ring_size, 1001, batch_size, nodeNet, eda_timer);

        // Rss_Open(res, intermediate_x[layer_ctr], 1001,  ring_size, nodeNet);

        gettimeofday(&end, NULL); // stop timer here

        // discarding the first two runs
        unsigned long long temp_time = 1e6 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;

        printf("%lu: time = %.3lf; eda = %.3lf;\n", k, (double)(temp_time * 1e-6), (double)(eda_timer * 1e-6));
        ...
        // Rss_Open(res, final_x_ind, 1,  ring_size, nodeNet);
