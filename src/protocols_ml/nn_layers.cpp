...
void Rss_depthwise_conv2d(Lint ***x, Lint ***W, Lint ***b, Lint **relu_bunds, Lint **trunc_vals, Lint **input_zp, Lint **weights_zp, Lint **output_zp, uint *n, uint *in_channels, uint out_channels, uint stride, uint batch_size, uint *layer_id, Lint public_M, uint ring_size,  NodeNetwork *nodeNet) {
    ...
    Rss_dw_matrixmul(x[*layer_id + 0], x_padded, W[*layer_id], *n, *n, (*in_channels), 3, ring_size, batch_size, stride, nodeNet);
    ...
    *n = *n / stride; // stride from previous layer
    ...
    // // activation
    ReLU_6_alpha(x[*layer_id + 0], x[*layer_id + 0], relu_bunds, (*in_channels) * (*n) * (*n), ring_size, *layer_id, nodeNet);

    Rss_truncPriv(x[*layer_id + 0], x[*layer_id + 0], trunc_vals, public_M, *layer_id, (*in_channels) * (*n) * (*n), ring_size, nodeNet);
...
void Rss_pointwise_conv2d(Lint ***x, Lint ***W, Lint ***b, Lint **relu_bunds, Lint **trunc_vals, Lint **input_zp, Lint **weights_zp, Lint **output_zp, uint *n, uint *in_channels, uint out_channels, uint stride, uint batch_size, uint *layer_id, Lint public_M, uint ring_size,  NodeNetwork *nodeNet) {
    ...
    // PROBLEM HERE
    Rss_pw_matrixmul(x[*layer_id + 0], x_padded, W[*layer_id], *n, *n, (*in_channels), 1, out_channels, ring_size, batch_size, nodeNet);

    pw_add_biases(x[*layer_id + 0], x[*layer_id + 0], b[*layer_id], *n, *n, out_channels, batch_size, nodeNet);

    // // activation
    ReLU_6_alpha(x[*layer_id + 0], x[*layer_id + 0], relu_bunds, out_channels * (*n) * (*n), ring_size, *layer_id, nodeNet);

    Rss_truncPriv(x[*layer_id + 0], x[*layer_id + 0], trunc_vals, public_M, *layer_id, out_channels * (*n) * (*n), ring_size, nodeNet);
...
void Rss_dw_pw_conv2d(Lint ***x, Lint ***W, Lint ***b, Lint **relu_bunds, Lint **trunc_vals, Lint **input_zp, Lint **weights_zp, Lint **output_zp, uint *n, uint *in_channels, uint out_channels, uint stride, uint batch_size, uint *layer_id, Lint public_M, uint ring_size,  NodeNetwork *nodeNet, unsigned long &timer) {
    ...
    *n = *n / stride; // stride from previous layer
    ...
    // activation
    ReLU_6_alpha_timer(x[*layer_id + 0], x[*layer_id + 0], relu_bunds, (*in_channels) * (*n) * (*n) * batch_size, ring_size, *layer_id, nodeNet, timer);

    Rss_truncPriv_time(x[*layer_id + 0], x[*layer_id + 0], trunc_vals, public_M, *layer_id, (*in_channels) * (*n) * (*n) * batch_size, ring_size, nodeNet, timer);
    ...
    *layer_id += 1;
    ...
    // // activation
    ReLU_6_alpha_timer(x[*layer_id + 0], x[*layer_id + 0], relu_bunds, out_channels * (*n) * (*n) * batch_size, ring_size, *layer_id, nodeNet, timer);

    Rss_truncPriv_time(x[*layer_id + 0], x[*layer_id + 0], trunc_vals, public_M, *layer_id, out_channels * (*n) * (*n) * batch_size, ring_size, nodeNet, timer);
...
void Rss_conv2d_first(Lint ***x, Lint **input_x, Lint ***W, Lint ***b, Lint **relu_bunds, Lint **trunc_vals, Lint **input_zp, Lint **weights_zp, Lint **output_zp, uint *n, uint kernel, uint *in_channels, uint out_channels, uint stride, uint batch_size, uint *layer_id, Lint public_M, uint ring_size,  NodeNetwork *nodeNet, unsigned long &timer) {
    ...
    *n = *n / stride;
    ...
    gettimeofday(&start, NULL); //start timer here
    Rss_pw_matrixmul(x[*layer_id], x_padded, W[*layer_id], *n, *n, (*in_channels), kernel, out_channels, ring_size, batch_size, nodeNet);
    gettimeofday(&end, NULL); //stop timer here
    op_timer = 1e6 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;
    ...
    gettimeofday(&start, NULL); //start timer here 
    ReLU_6_alpha_timer(x[*layer_id], x[*layer_id], relu_bunds, out_channels * (*n) * (*n) * batch_size, ring_size, *layer_id, nodeNet, timer); 
    gettimeofday(&end, NULL); //stop timer here
    op_timer = 1e6 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;
    // printf("ReLU6 time = %.6lf\n", (double)(op_timer * 1e-6));

    gettimeofday(&start, NULL); //start timer here
    Rss_truncPriv_time(x[*layer_id], x[*layer_id], trunc_vals, public_M, *layer_id, out_channels * (*n) * (*n) * batch_size, ring_size, nodeNet, timer);
    gettimeofday(&end, NULL); //stop timer here
    op_timer = 1e6 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec;
...
void Rss_conv2d(Lint ***x, Lint ***W, Lint ***b, Lint **relu_bunds, Lint **trunc_vals, Lint **input_zp, Lint **weights_zp, Lint **output_zp, uint *n, uint kernel, uint *in_channels, uint out_channels, uint stride, uint batch_size, uint *layer_id, Lint public_M, uint ring_size,  NodeNetwork *nodeNet) {
    ...
    *n = *n / stride; // DOUBLE CHECK THAT THIS IS RIGHT
    ...
    Rss_pw_matrixmul(x[*layer_id + 1], x_padded, W[*layer_id], *n, *n, (*in_channels), kernel, out_channels, ring_size, batch_size, nodeNet);
    ...
    ReLU_6_alpha(x[*layer_id + 1], x[*layer_id + 1], relu_bunds, out_channels * (*n) * (*n) * batch_size, ring_size, *layer_id, nodeNet);

    Rss_truncPriv(x[*layer_id + 1], x[*layer_id + 1], trunc_vals, public_M, *layer_id, out_channels * (*n) * (*n) * batch_size, ring_size, nodeNet);
...
void Rss_conv2d_final_layer(Lint ***x, Lint **output_x, Lint ***W, Lint ***b, Lint **relu_bunds, Lint **trunc_vals, Lint **input_zp, Lint **weights_zp, Lint **output_zp, uint *n, uint kernel, uint *in_channels, uint out_channels, uint stride, uint batch_size, uint *layer_id, Lint public_M, uint ring_size,  NodeNetwork *nodeNet, unsigned long &timer) {
    ...
    *n = *n / stride;
    ...
    Rss_pw_matrixmul(output_x, x_padded, W[*layer_id], *n, *n, (*in_channels), kernel, out_channels, ring_size, batch_size, nodeNet);
    ...
    ReLU_6_alpha_timer(output_x, output_x, relu_bunds, out_channels * (*n) * (*n) * batch_size, ring_size, *layer_id, nodeNet, timer);

    // cout<<"relu_6  end "<<endl;
    Rss_truncPriv_time(output_x, output_x, trunc_vals, public_M, *layer_id, out_channels * (*n) * (*n) * batch_size, ring_size, nodeNet, timer);
...
void Rss_conv2d_no_act(Lint **res, Lint **x, Lint **W, Lint **b, Lint **relu_bunds, uint *n, uint kernel, uint *in_channels, uint out_channels, uint stride, uint ring_size, uint batch_size, uint *layer_id,  NodeNetwork *nodeNet) {
    ...
    *n = *n / stride; // DOUBLE CHECK THAT THIS IS RIGHT
    ...
    Rss_pw_matrixmul(res, x_padded, W, *n, *n, (*in_channels), kernel, out_channels, ring_size, batch_size, nodeNet);
