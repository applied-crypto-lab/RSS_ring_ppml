...
void new_Rss_Convert_mp(Lint **res, Lint **a, uint size, uint ring_size, uint ring_size_prime, NodeNetwork *nodeNet) {
    ...
    Rss_RandBit_mp(r_shares, n_rand_bits, ring_size_prime, nodeNet);
    ...
    // Rss_Open_mp(c, sum, size, ring_size_prime, nodeNet);
    Rss_Open_mp(c, sum, size, ring_size, nodeNet);

    Rss_BitAdd_mp(a_2, c, edaBit_b_2, ring_size, size, nodeNet);
    // Rss_BitAdd_mp(res, c, edaBit_b_2, ring_size, size, nodeNet);
...
void Rss_Convert_mp(Lint **res, Lint **a, uint size, uint ring_size, uint ring_size_prime, NodeNetwork *nodeNet) {
    ...
    Rss_BitAdd_mp(a_2, c, r_2, ring_size, size, nodeNet);

    // Rss_Open_Bitwise_mp(res_check, a_2, size, ring_size, nodeNet);
    // for (i = 0; i < size; i++) {
