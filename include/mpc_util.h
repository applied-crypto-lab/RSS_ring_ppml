...
// double getScale(uint bound, vector<double> input);
double getScale(uint bound, std::vector<double> x);

inline __attribute__((always_inline)) Lint GET_BIT(Lint X, Lint N) {
    return (((X) >> (N)) & Lint(1));
}
inline __attribute__((always_inline)) Lint SET_BIT(Lint X, Lint N, Lint B) {
    return (X & ~(Lint(1) << N)) | (B << N);
}
